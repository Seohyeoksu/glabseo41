...
if menu == "📋 공모전 개요":
                ...
                with col1:
                    if st.form_submit_button("로그인", use_container_width=True):
                        ...
                        if (
                            ...
                            == hashlib.sha256("thohkom1566*".encode()).hexdigest()
                        ):
    ...
    # 일반 사용자 모드
        ...
        with col2:
            questions = len(
                [c for c in st.session_state.comments if c["type"] == "질문"]
            )
            st.metric("질문", f"{questions}개")
        with col3:
            ideas = len(
                [c for c in st.session_state.comments if c["type"] == "아이디어"]
            )
        ...
        st.warning(
            "⚠️ 현재는 임시 저장 방식으로, 페이지를 새로고침하면 내용이 사라집니다."
        )
